...
public class AdminActivity extends Activity{
 ...
 private void loadMenuPrices(){if(!isAdmin(()->{}))return;db.collection("menu").get().addOnSuccessListener(s->{try{JSONArray a=new JSONArray();for(DocumentSnapshot d:s){JSONObject o=new JSONObject();o.put("id",d.getId());o.put("name",d.getString("name")==null?itemName(d.getId()):d.getString("name"));o.put("price",num(d.get("price")));o.put("enabled",!Boolean.FALSE.equals(d.getBoolean("enabled")));a.put(o);}js("window.menuResult("+JSONObject.quote(a.toString())+")");}catch(Exception e){message("Menu data error: "+e.getMessage());}}).addOnFailureListener(e->message("Could not load menu: "+e.getMessage()));}
