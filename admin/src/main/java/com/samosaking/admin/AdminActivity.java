...
public class AdminActivity extends Activity{
 ...
 private void updateStatus(String id,String st){if(id==null||id.trim().isEmpty()){message("Invalid order ID.");return;}if(st==null||st.trim().isEmpty()){message("Invalid status.");return;}message("Updating order "+id+" → "+st+" …");isAdmin(()->db.collection("orders").document(id).update("status",st).addOnSuccessListener(v->message("✓ Order "+id+" → "+st)).addOnFailureListener(e->message("✗ Status update failed: "+e.getClass().getSimpleName()+" — "+e.getMessage())));}
